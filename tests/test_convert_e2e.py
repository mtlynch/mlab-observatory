...
def _diff_dirs(left_dir, right_dir):
    ...
    for right_only_file in dir_cmp.right_only:
        ...
        for diff in unified_diff([], open(right_path).readlines(), left_path,
                                 right_path):
            diffs.append(diff)
...
class ResultConverterEndToEnd(unittest.TestCase):
    ...
    def setUp(self):
        clear_output_dir()
        self._input_filenames = glob.glob(os.path.join(TESTDATA_DIR,
                                                       '*-raw.csv'))
    ...
    def _create_per_site_converter(self):
        ...
        return convert.ResultConverter(grouper, self._median_reducer,
                                       self._file_writer, output_dir,
                                       valid_keys_path)
