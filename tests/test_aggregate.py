...
class AggregateTest(unittest.TestCase):
    ...
    def test_aggregate_by_month_during_daylight_saving_time_switch(self):
        """Verify that days in DST and days in non-DST are aggregated together."""
        results = [
            (make_date(2013, 3, 6, 3, 45, 58), 1.1),  # Non-DST
            (make_date(2013, 3, 15, 23, 45, 58), 2.2),  # DST
            (make_date(2013, 11, 1, 3, 45, 58), 3.3),  # DST
            (make_date(2013, 11, 5, 23, 45, 58), 4.4),  # Non-DST
        ]
        aggregated_expected = {
    ...
    def test_aggregate_by_hour_of_day_per_month(self):
        ...
        aggregated_actual = aggregate.aggregate_by_hour_of_day_per_month(
            results)
        self.assertDictEqual(aggregated_expected, aggregated_actual)
