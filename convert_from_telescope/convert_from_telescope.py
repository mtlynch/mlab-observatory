...
def perform_conversion(input_filenames, output_dir):
    ...
    per_site_valid_keys_path = os.path.join(output_dir, 'metadata',
                                            'validExploreKeys.txt')
    per_site_converter = convert.ResultConverter(
        per_site_result_grouper, median_reducer, file_writer,
        per_site_output_dir, per_site_valid_keys_path)
    ...
    per_metro_valid_keys_path = os.path.join(output_dir, 'metadata',
                                             'validCompareKeys.txt')
    per_metro_converter = convert.ResultConverter(
        per_metro_result_grouper, median_reducer, file_writer,
        per_metro_output_dir, per_metro_valid_keys_path)
...
if __name__ == '__main__':
    ...
    parser.add_argument('-o',
                        '--output',
                        default='../static/observatory/',
                        help='Output path.')
    parser.add_argument('--samples_per_day',
                        default='30',
                        help='Minimum number of samples required per day.')
    parser.add_argument('--percentage_valid_days',
                        default='0.80',
    ...
    parser.add_argument('--no_whitelist_update',
                        default=False,
                        action='store_true',
